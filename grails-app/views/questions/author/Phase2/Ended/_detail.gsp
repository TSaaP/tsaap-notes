...
<div class="question" id="question_${note.id}">
    ...
    <g:if test="${note.context.isOpen()}">
        ${message(code: "questions.author.phase2.ended.close")} ${sessionPhase.responseCount()} ${message(code: "questions.author.responses")} >
        <g:remoteLink action="startPhase" controller="question"
                      params="[liveSessId: sessionPhase.liveSession.id, noteId: note.id, phaseRank: 3]"
                      class="btn btn-success btn-xs" update="question_${note.id}"
                      onComplete="MathJax.Hub.Queue(['Typeset',MathJax.Hub,'question_${note.id}'])">
            <span class="glyphicon glyphicon-play"></span> ${message(code: "questions.author.phase2.ended.start.phase3")}</g:remoteLink>
    </g:if>
</div>
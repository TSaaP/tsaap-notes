...
<g:if test="${sequence.state == StateType.show.name()}">
  <g:if test="${sequence.executionIsFaceToFace()}">
    <g:if
      ...
      <g:render
          template="/assignment/player/sequence/teacher/sequenceInfo/phaseState/${activeInteractionState}-elaastic"
          model="[interactionInstance: activeInteraction]"/>
    </g:if>
    <g:elseif test="${activeInteraction.interactionType == InteractionType.Read.name()}">
      <g:render template="/assignment/player/Read/teacher/result_publication_status"
                model="[interactionInstance: activeInteraction]"/>
    </g:elseif>
  </g:if>
  <g:else>
